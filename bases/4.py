...
class Mover:
    def __init__(self, pos_vector, v_vector, a_vector, m, g_force, w_force): # *args = forces acting on the object
        self.pos = pos_vector
        self.v = v_vector
        self.a = a_vector
        self.m = m
        self.g_force = g_force.calculate(m)
        self.w_force = w_force.calculate()
    
    def update(self, t):
        # f = self.gravity_force_vector()
        self.a.x_component = (self.g_force.x_component+self.w_force.x_component) / self.m
        self.a.y_component = (self.g_force.y_component+self.w_force.y_component) / self.m
...
mover = Mover(Vector(60*width/2, 0), Vector(0, 0), Vector(0, 0), 10, GravityForce(0, 0), WindForce(0, 0)) # THE MASS DOES NOT AFFECT THE SPEED (MOVEMENT) HERE, WHICH IS ABSOLUTELLY RIGHT. (F = ma)
