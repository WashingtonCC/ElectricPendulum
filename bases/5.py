...
class ChargedMover:
    def __init__(
        self, pos_vector, v_vector, a_vector, m, g_force, e_force
    ):  # *args = forces acting on the object
        self.pos = pos_vector
        self.v = v_vector
        self.a = a_vector
        self.m = m
...
def draw(bg):
    bg.fill((0, 0, 0))
    pygame.draw.circle(
        bg, (255, 255, 255), (mover.pos.x_component, mover.pos.y_component), 10
    )
    pygame.draw.circle(
        bg,
        (255, 255, 255),
        (static_charge.pos.x_component, static_charge.pos.y_component),
        8,
    )
...
static_charge = StaticCharge(
    Vector(60 * width / 2, 60 * (height - 20)), 2 * math.pow(10, -2)
)
...
while True:
    ...
    mover.update(
        t, distance_angle(mover, static_charge), static_charge.q, static_charge.q
    )
    draw(screen)
